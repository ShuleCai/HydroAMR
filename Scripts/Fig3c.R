...
term_year <- read.csv("./Data/Fig3c.csv", row.names = 1, check.names = F)
...
p1 <-
  p0 + rowAnnotation(
    Trend = anno_horizon(term_year %>% t(),
      normalize = T,
      width = unit(12, "mm"),
      axis = F
    ),
  ...
  rowAnnotation(
    Occurrence = anno_barplot(term_year %>% rowSums(),
      axis_param = list(
        gp = gpar(fontsize = 9),
        side = "top"
      )
...
pdf("./Figures/Fig3c.pdf", family = "Times", height = 8, width = 11)
p1
dev.off()
