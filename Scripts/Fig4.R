...
plot_data <- df_topics_c_m %>% select(individual = CustomName, group = Topic.category, value = Count) %>% 
...
allplotdata <- tibble('group' = rep(datagroup, 2),
                      'individual' = paste0('empty_individual_', 1:8),
                      'value' = 0) %>% 
  bind_rows(plot_data) %>% arrange(desc(group), desc(value)) %>% mutate(xid = 1:n()) %>% 
...
coordy <- tibble('coordylocation' = seq(from = min(allplotdata$value), to = max(allplotdata$value), 1),
...
ggplot() + 
  ...
  #           aes(x = xid, label = individual, y = value+0.1, angle = 0, hjust = hjust),
  #           color="black", alpha=1, size=2.5) +
  geom_text(data = allplotdata %>% filter(!str_detect(individual, pattern = "empty_individual")),
            aes(x = xid, label = individual, y = value+0.1, angle = angle, hjust = hjust),
            color="black",alpha=1, size=2.5) +
  # geom_segment(data = segment_data, aes(x = from, xend = to), y = -2, yend=-2) +
  # geom_text(data = segment_data, aes(x = labelx, label = label), y = -2) +
  # geom_text(data = coordy, aes(x = x, y = coordylocation+0.5, label = coordytext),
...
plot_data <- plot_data %>% mutate(topic_category = factor(topic_category, levels = c("AMR_surveillance", "AMR_control_removal", 
                                                                                     "aquaculture_health", "antmicrobial_agent") %>% rev))
...
ggsave("./Figures/Fig4d.pdf", family = "ArialMT",
       height = 2.5,
       width = 10)
