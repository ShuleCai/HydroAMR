...
plot_data_pct1 <- read.csv("./Data/Fig5e.csv", check.names = F, row.names = 1)
plot_data_pct2 <- read.csv("./Data/Fig5f.csv", check.names = F, row.names = 1)
total1 <- read.csv("./Data/Fig5e_total.csv", check.names = F, row.names = 1)
total2 <- read.csv("./Data/Fig5f_total.csv", check.names = F, row.names = 1)

# Create the first percentage bar chart
p1_data <- data.frame(
  category = "Hydrocycle\nproportion",
  status = c("Yes", "No"),
  percentage = c(2.652425, 97.347575)
)
pic_line_width <- 0.2
...
p2_data <- data.frame(
  category = "Hydro-only",
  status = c("Yes", "No"),
  percentage = c(35.48397, 64.51603)
)
