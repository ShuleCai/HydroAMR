...
mk_df <- read.csv("./Data/Fig3d.csv", row.names = 1, check.names = F)
...
grid.text("Legend",
  x = legend_x, y = legend_y - unit(0.3, "cm"),
  just = "left", gp = gpar(fontsize = 9, fontface = "bold")
)

# Red square and label
grid.rect(
  x = legend_x + unit(0.2, "cm"), y = legend_y - unit(0.6, "cm"),
  width = unit(0.3, "cm"), height = unit(0.3, "cm"),
  gp = gpar(fill = "darkred", col = NA)
)
grid.text("*",
  x = legend_x + unit(0.35, "cm"), y = legend_y - unit(0.6, "cm"),
  gp = gpar(fontsize = 8)
)

# Blue square and label
grid.rect(
  x = legend_x + unit(0.2, "cm"), y = legend_y - unit(0.9, "cm"),
  width = unit(0.3, "cm"), height = unit(0.3, "cm"),
  gp = gpar(fill = "darkblue", col = NA)
)
grid.text("△",
  x = legend_x + unit(0.35, "cm"), y = legend_y - unit(0.9, "cm"),
  gp = gpar(fontsize = 8)
)
...
# total group label
grid.text("total",
  x = unit(0.04, "npc"), y = unit(0.98, "npc"),
  gp = gpar(fontsize = 10, fontface = "bold")
)

# Income group label
grid.text("Income",
  x = unit(0.18, "npc"), y = unit(0.98, "npc"),
  gp = gpar(fontsize = 10, fontface = "bold")
)

# Region group label
grid.text("Region",
  x = unit(0.55, "npc"), y = unit(0.98, "npc"),
  gp = gpar(fontsize = 10, fontface = "bold")
)
dev.off()
